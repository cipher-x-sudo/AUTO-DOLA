...
async def process_video(session: Session, job: Job) -> None:
    ...
    for item in session.exec(select(JobItem).where(JobItem.job_id == job.id)).all():
        if job.status == JobStatus.cancelled:
            mark_item(session, item, ItemStatus.cancelled, "Cancelled")
            continue
        try:
            mark_item(session, item, ItemStatus.running, "Building Dola session")
            dola_session = await client.build_session()
            payload = build_dola_payload(dola_session.payload_template, item.prompt, config.get("duration", 15), config.get("ratio", "9:16"))
            mark_item(session, item, ItemStatus.running, "Submitting Seedance request")
            conversation_id, conversation_type = await client.submit(dola_session, payload)
            mark_item(session, item, ItemStatus.running, "Polling video id")
            ...
            mark_item(session, item, ItemStatus.running, "Polling download URL")
            download_url = await client.poll_download_url(dola_session, vid)
            if not download_url:
                raise RuntimeError("Timed out waiting for video download URL.")
            filename = safe_filename(item.title or f"video-{item.id.hex[:8]}")
            ...
            if config.get("clean_watermark", True):
                ...
                shutil.copyfile(raw_path, final_path)
            artifact = Artifact(job_id=job.id, item_id=item.id, kind="video", path=str(final_path), filename=final_path.name, mime_type="video/mp4", size_bytes=final_path.stat().st_size)
            add_artifact(session, artifact, item)
            mark_item(session, item, ItemStatus.completed, final_path.name)
            log(session, f"Completed video: {final_path.name}", "success", job.id)
        except Exception as exc:
            mark_item(session, item, ItemStatus.failed, "Failed", str(exc))
            log(session, f"Item failed: {exc}", "error", job.id)
